...
const FavoriteActionSchema: Schema = {
    type: "object",
    properties: {
...
async function favoriteAction( data: IFavoriteAction ): Promise<boolean> {

    try {

        if ( !ajv.validate(FavoriteActionSchema, data) ) {
        ...
        if (resUpdate.success) {
            app.log.info(`Article with ID ${item_id} added to favorite successfully.`);
            return true;
        }

        app.log.error(`Failed to favorite article with ID ${item_id}: ${resUpdate.message}`);
        return false;
