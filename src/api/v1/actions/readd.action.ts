/**
 * @description Unarchive an article by setting it's status to 0
 * see: https://abdulrahim2002.github.io/open-pocket-docs/docs/API-spec/Endpoints/send/#action-readd
 */
...
interface IReaddAction {
    item_id:    string,
};


const readdActionSchema: Schema = {
    type: "object",
    properties: {
...
async function readdAction( data: IReaddAction ): Promise<boolean> {

    try {

        if ( !ajv.validate(readdActionSchema, data) ) {
        ...
        if (resUpdate.success) {
            app.log.info(`Article with ID ${item_id} readded successfully.`);
            return true;
        }

        app.log.error(`Failed to readd article with ID ${item_id}: ${resUpdate.message}`);
        return false;
