...
const archiveActionSchema: Schema = {
    type: "object",
    properties: {
...
async function archiveAction( data: IArchiveParams ): Promise<boolean> {
    ...
    try {
        ...
        if ( !ajv.validate(archiveActionSchema, data) ) {
        ...
        if (resUpdate.success) {
            app.log.info(`Article with ID ${item_id} archived successfully.`);
            return true;
        }

        app.log.error(`Failed to archive article with ID ${item_id}: ${resUpdate.message}`);
        return false;
