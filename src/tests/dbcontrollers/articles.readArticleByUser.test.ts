...
const testUserForReadArticleByUser: typeof usersSchema.$inferInsert = {
    provider: "open-pocket",
    name: "test-user-read-article-user-by-user",
    email: "test-user-read-article-by-user@mail.com",
    hashed_password: "on124ho13ih482h9h2398hd289h993hr293g"
};

test("readArticleByUser", async () => {
    // first create a test user
    const resCreateUser = await createUser(testUserForReadArticleByUser);
    ...
    // now find the article using user_id
    const resReadArticlesByUser = await readArticlesByUser(resCreateUser.data!.user_id);
    ...
    expect(resReadArticlesByUser).toMatchObject({
        success: true,
        status: OPSTATUS.SUCCESS,
        message: `Articles retrieved for user: ${resCreateUser.data!.user_id}`,
        recommendedHttpResponseCode: StatusCodes.OK,
        data: resReadArticlesByUser.data,
    });
});
